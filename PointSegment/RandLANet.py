...
# from main_BraTS20_v1 import phase
...
class Network:
    ...
    def dice_loss(self, logits, labels, pre_cal_weights):
        one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)
        # set_weights = False
        # if set_weights:
        #     pre_cal_weights = np.array([1,1,1,1])
        #     class_weights = tf.convert_to_tensor(pre_cal_weights, dtype=tf.float32)
        #     one_hot_labels = tf.reduce_sum(class_weights * one_hot_labels, axis=1)

        dice_numerator = 2.0 * tf.reduce_sum(
        one_hot_labels * logits,axis=0)
        dice_denominator = \
            tf.reduce_sum(tf.square(logits), axis=0) + \
            tf.reduce_sum(one_hot_labels, axis=0)
    ...
    def get_loss_dice_weight(self, logits, labels, pre_cal_weights):
        one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)
        class_weights = tf.convert_to_tensor(np.array([[4,1,1,1]]), dtype=tf.float32)
        weight_map_nclasses = class_weights*one_hot_labels

        dice_numerator = 2.0 * tf.reduce_sum(
        class_weights*one_hot_labels * logits,axis=0)


        dice_denominator = \
            tf.reduce_sum(class_weights*tf.square(logits), axis=0) + \
            tf.reduce_sum(one_hot_labels, axis=0)
        epsilon_denominator = 0.00001
