...
class ConfigBraTS:
    ...
    train_steps = 295  # Number of steps per epochs
    val_steps = 74  # Number of validation steps per epoch
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        num_per_class = []
        if dataset_name is 'BraTS20':
            num_per_class = np.array([1, 1, 1, 1])
        elif dataset_name is 'BraTS_Block64':
            num_per_class = np.array([1403, 22, 80, 11])
        elif dataset_name is 'Pancreas':
            num_per_class = np.array([1, 1])
