    ...
    def inputs(self):
        ...
        if self.modelType == 'training':
            if config.MIXUP:
                ...
                """
                #Facebook mixup style
                ret = [
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, S[0], S[1], S[2], 4), 'image'),
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, S[0], S[1], S[2], 1), 'weight'),
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, S[0], S[1], S[2], 4), 'label'),
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, S[0], S[1], S[2], 4), 'label_mix'),
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, 1), 'gamma'),]
                """
            else:
                ret = [
                    tf.placeholder(tf.float32, (config.BATCH_SIZE, S[0], S[1], S[2], 4), 'image'),
    ...
    def build_graph(self, *inputs):
        ...
        if is_training:
            if config.MIXUP:
                ...
                #Facebook mixup style
                #image, weight, label, label_mix, gamma = inputs
            else:
                image, weight, label = inputs
        else:
            image = inputs[0]
        ...
        # image = tf.transpose(tf.constant(np.zeros((config.BATCH_SIZE,128,128,128,8)).astype(np.float32)), [0,4,1,2,3])
        ...
        if is_training:
            ...
            if config.MIXUP:
                ...
                #Facebook mixup style
                #loss = Loss_mixup(featuremap, weight, label, label_mix, gamma[0][0])
            else:
                loss = Loss(featuremap, weight, label)
        ...
        cfg = AutoResumeTrainConfig(
            ...
            callbacks=[
                ...
                ScheduledHyperParamSetter('learning_rate', 
                    [(25, config.BASE_LR*0.5),
                    (50, config.BASE_LR*0.1),
                    (65, config.BASE_LR*0.05),
                    (80, config.BASE_LR*0.01),
                    (105, config.BASE_LR*0.005),
                    (120, config.BASE_LR*0.001),
                    (130, config.BASE_LR*0.0005),
                    (140, config.BASE_LR*0.0001),
                    (145, config.BASE_LR*0.00005),
                    (150, config.BASE_LR*0.00001)]
                ),
                #EvalCallback(),
