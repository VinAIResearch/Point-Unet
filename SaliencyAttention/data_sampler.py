        ...
        for data in self.ds.get_data():
            ...
            if config.MIXUP:
                if len(holder) == self.batch_size + 1:
                    ...
                    yield BatchData._aggregate_batch(mixup_samples, self.use_list)
                    del holder[:]

            elif len(holder) == self.batch_size:
            ...
            if len(holder) == self.batch_size:
                if config.MIXUP:
                    mixup_samples = mixup_batch(holder)
                    yield BatchData._aggregate_batch(mixup_samples, self.use_list)
                    del holder[:]
                else:
                    yield BatchData._aggregate_batch(holder, self.use_list)
...
def get_test_dataflow():
    ...
    if isinstance(config.BASEDIR, (list, tuple)):
            ...
            for basedir in config.BASEDIR:
                imgs.extend(BRATS_SEG.load_many(
                    basedir, config.TEST_DATASET, add_gt=False))

    else:
        imgs = BRATS_SEG.load_many(
            config.BASEDIR, config.TEST_DATASET, add_gt=False)
    # no filter for training
    if config.NO_CACHE:
    ...
    def preprocess(data):
        im = data[-1]
        volume_list, label, weight, original_shape, bbox = crop_brain_region(im, None, with_gt=False)
        batch = sampler3d_whole(volume_list, label, weight, original_shape, bbox)
